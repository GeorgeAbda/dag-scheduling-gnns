...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train DAG scheduling agent')
    ...
    if hasattr(train_args, 'exp_name'): train_args.exp_name = e.get('name', train_args.exp_name)
    if hasattr(train_args, 'seed'): train_args.seed = e.get('seed', train_args.seed)
    if hasattr(train_args, 'output_dir'): train_args.output_dir = e.get('output_dir', train_args.output_dir)
    if hasattr(train_args, 'device'): train_args.device = e.get('device', train_args.device)
    
    # Training
    t = cfg.get('training', {})
    if hasattr(train_args, 'total_timesteps'): train_args.total_timesteps = t.get('total_timesteps', train_args.total_timesteps)
    if hasattr(train_args, 'learning_rate'): train_args.learning_rate = t.get('learning_rate', train_args.learning_rate)
    if hasattr(train_args, 'num_envs'): train_args.num_envs = t.get('num_envs', train_args.num_envs)
    if hasattr(train_args, 'num_steps'): train_args.num_steps = t.get('num_steps', train_args.num_steps)
    if hasattr(train_args, 'gamma'): train_args.gamma = t.get('gamma', train_args.gamma)
    if hasattr(train_args, 'gae_lambda'): train_args.gae_lambda = t.get('gae_lambda', train_args.gae_lambda)
    if hasattr(train_args, 'num_minibatches'): train_args.num_minibatches = t.get('num_minibatches', train_args.num_minibatches)
    if hasattr(train_args, 'update_epochs'): train_args.update_epochs = t.get('update_epochs', train_args.update_epochs)
    if hasattr(train_args, 'clip_coef'): train_args.clip_coef = t.get('clip_coef', train_args.clip_coef)
    if hasattr(train_args, 'ent_coef'): train_args.ent_coef = t.get('ent_coef', train_args.ent_coef)
    if hasattr(train_args, 'vf_coef'): train_args.vf_coef = t.get('vf_coef', train_args.vf_coef)
    if hasattr(train_args, 'max_grad_norm'): train_args.max_grad_norm = t.get('max_grad_norm', train_args.max_grad_norm)
    if hasattr(train_args, 'anneal_lr'): train_args.anneal_lr = t.get('anneal_lr', train_args.anneal_lr)
    
    # Evaluation
    ev = cfg.get('evaluation', {})
    if hasattr(train_args, 'test_every_iters'): train_args.test_every_iters = ev.get('test_every_iters', train_args.test_every_iters)
    if hasattr(train_args, 'robust_eval_alpha'): train_args.robust_eval_alpha = ev.get('robust_eval_alpha', train_args.robust_eval_alpha)
    
    # Domain
    d = cfg.get('domain', {})
    if hasattr(train_args, 'longcp_config'): train_args.longcp_config = d.get('longcp_config')
    if hasattr(train_args, 'wide_config'): train_args.wide_config = d.get('wide_config')
    
    # Seed control
    sc = cfg.get('seed_control', {})
    if hasattr(train_args, 'training_seed_mode'): train_args.training_seed_mode = sc.get('mode', train_args.training_seed_mode)
    if hasattr(train_args, 'train_seeds_file'): train_args.train_seeds_file = sc.get('seeds_file')
    
    # Variant
    v = cfg.get('variant', {})
    if hasattr(train_args, 'variant_name'): train_args.variant_name = v.get('name', train_args.variant_name)
    
    # Trajectory
    tr = cfg.get('trajectory', {})
    if hasattr(train_args, 'collect_trajectory'): train_args.collect_trajectory = tr.get('enabled', train_args.collect_trajectory)
    if hasattr(train_args, 'trajectory_collect_every'): train_args.trajectory_collect_every = tr.get('collect_every', train_args.trajectory_collect_every)
    if hasattr(train_args, 'trajectory_method'): train_args.trajectory_method = tr.get('method', train_args.trajectory_method)
    
    # Logging
    lg = cfg.get('logging', {})
    if hasattr(train_args, 'track'): train_args.track = lg.get('tensorboard', train_args.track)
    if hasattr(train_args, 'log_every_n_updates'): train_args.log_every_n_updates = lg.get('log_every', train_args.log_every_n_updates)
